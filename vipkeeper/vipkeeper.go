...
func (v *VipKeeper) campaign() {
	for {
		...
		case <-s.Done(): // 如果因为网络因素导致与etcd断开了keepAlive，这里break，重新创建session，重新选举
			...
			break
		case <-v.ctx.Done():
			ctxTmp, _ := context.WithTimeout(context.Background(), time.Millisecond*time.Duration(v.conf.Interval*5))
			err := e.Resign(ctxTmp)
			if err != nil {
...
/*
initial-advertise-peer-urls http://localhost:23801 --listen-peer-urls http://0.0.0.0:2380 \
  --advertise-client-urls http://localhost:23791   --listen-client-urls http://0.0.0.0:2379 \
